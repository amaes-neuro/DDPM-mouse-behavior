# -*- coding: utf-8 -*-
"""
Created on Thu Feb 20 16:23:17 2025

Visualize the grid, only the effect of TMT when food is present.
...
for i in range(nb_points):
    ...
    for h in range(np.shape(samples)[0]):
        ...
        for j in range(8):     
            total[j] = np.mean( np.sqrt( (samples[h,:,j+8,5,0]-coords[grid_id,0])**2 + (samples[h,:,j+8,5,1]-coords[grid_id,1])**2 ) )
            delta[j,0] = np.mean( np.sqrt( (samples[h,:,j+8,0,0]-coords[grid_id,0])**2 + (samples[h,:,j+8,0,1]-coords[grid_id,1])**2 ) )
            delta[j,1] = np.std(np.sqrt( (samples[h,:,j+8,0,0]-coords[grid_id,0])**2 + (samples[h,:,j+8,0,1]-coords[grid_id,1])**2 ) )
            delta_angle[:,j,0] = np.sqrt( (samples[h,:,j+8,0,0]-coords[grid_id,0])**2 + (samples[h,:,j+8,0,1]-coords[grid_id,1])**2 )
            delta_angle[:,j,1] = np.arctan2( (samples[h,:,j+8,1,1]-coords[grid_id,1])/(samples[h,:,j+8,1,0]-coords[grid_id,0]) )
            smpl_avg[h,j,0,0] = np.mean(samples[h,:,j+8,0,0]-coords[grid_id,0] )
            smpl_avg[h,j,0,1] = np.mean(samples[h,:,j+8,0,1]-coords[grid_id,1] )
            for k in range(1,6):
                delta[j,0] += np.mean( np.sqrt( (samples[h,:,j+8,k,0]-samples[h,:,j,k-1,0])**2 + (samples[h,:,j+8,k,1]-samples[h,:,j,k-1,1])**2 ) )
                delta[j,1] += np.std(np.sqrt( (samples[h,:,j+8,k,0]-samples[h,:,j,k-1,0])**2 + (samples[h,:,j+8,k,1]-samples[h,:,j,k-1,1])**2 ) )
                delta_angle[:,j,0] += np.sqrt( (samples[h,:,j+8,k,0]-samples[h,:,j,k-1,0])**2 + (samples[h,:,j+8,k,1]-samples[h,:,j,k-1,1])**2 ) 
                smpl_avg[h,j,k,0] = np.mean(samples[h,:,j+8,k,0] -coords[grid_id,0])
                smpl_avg[h,j,k,1] = np.mean(samples[h,:,j+8,k,1] -coords[grid_id,1])
...
for j in range(3):
    for i in range(samples.shape[1]-50):
        ax[j].plot(np.insert(samples[8,i,8+3*j,:,0],0,coords[grid_id,0]),np.insert(samples[8,i,8+3*j,:,1],0,coords[grid_id,1]),c='yellowgreen')
...
for j in range(3):
    for i in range(samples.shape[1]-50):
        ax[j].plot(np.insert(samples[14,i,8+3*j,:,0],0,coords[grid_id,0]),np.insert(samples[14,i,8+3*j,:,1],0,coords[grid_id,1]),c='yellowgreen')
    ax[j].plot(*closed_box.xy,color='black')
    ax[j].axis('off')
plt.savefig('figures/grid_sweep/'+model+'/'+'trajectory_FOOD_'+str(file_nb1)+'_'+str(file_nb2)+'.pdf', format="pdf", bbox_inches="tight")
...
"""
"""
#concatenate the 7 2sec average trajectories to one feature vector (84D), for each location and 10 histories (1592 vectors or so)
...
X = all_data.reshape((all_data.shape[0],84))
...
plt.savefig('figures/grid_sweep/'+model+'/'+'trajectory_clusters_FOOD.pdf', format="pdf", bbox_inches="tight")
plt.close()
"""