# -*- coding: utf-8 -*-
"""
Created on Thu Oct  3 15:00:12 2024

Loop over a list of coordinates, and generate short 2 second trajectories starting
at each coordinate (includes moving average). Do this for a range of TMT values and food presence or not.
...
if not os.path.exists('data_model_curves/'+model+'/samples_sweep'):
    os.makedirs('data_model_curves/'+model+'/samples_sweep')
...
curves = np.zeros((len(coords),2*len(threat_values),time_steps,2))
...
#save curve    
data_dict = {'state': coords, 'actions': curves}    
print('Save sampled actions in state:',data_dict['state'])
with open('data_model_curves/'+model+'/samples_sweep/curve_'+sample+'.pickle', 'wb') as file:
